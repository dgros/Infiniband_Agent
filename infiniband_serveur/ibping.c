...
void Usage(char* name)
{
	...
	printf("Detection : %s hostname hostname.log (hostname_rep.log|NULL) 0 \n", name);
	printf("Protection : %s hostname hostname.log hostname_rep.log 1 \n", name);
}
...
void * thread_get()
{
  ...
  // Connection IB
        ...
        ibping_address_exchange(hostname);
        ibping_connection_connect(&ib_globals.peer_addr);
        fprintf(stderr, "remote LID = 0x%02X\n", ib_globals.peer_addr.lid);
      
        if(file_read == NULL) exit(0);

  // Pipe Entree 
        mknod(file_read, 777, S_IFIFO);
        file=fopen(file_write, "w+");

        if(file_read == NULL) exit(0);

  // Pipe de Sortie (ou reponse)
        int mk_ret=    mknod(file_read, 777, S_IFIFO);
        int op_read = open(file_read, O_RDONLY);
        if(op_read == -1 )
        {
                printf("Error opening read pipe \n");
                abort();
        }

        printf("je suis le recepteur\n");
        while(1)
        {
                ...
                fprintf(file, "%s", ib_globals.buffer.reception.data);
                fflush(file);
                char*temp= strtok(ib_globals.buffer.reception.data," ");
                ID = strtok(temp,"=");
                ID = strtok(NULL,"=");
                ib_globals.buffer.reception.busy = 0;
                if(ID == NULL)
                ...
                read_ret = 0;
                {
                       ...
                       if(read_ret > 0) 
                        {
                                strcpy(ligne,line);
                                temporaire = strtok(line, ":");
                                printf("temporaire bb%sbb\n", temporaire);
                                if(strcmp(temporaire,ID)==0)
                                {
                                        boolean = 1;
                                        printf("match found \n");
                                }
                ...
                size=sizeof(ligne);
                memcpy(ib_globals.buffer.emission.data, ligne, size);
                const char*const base  = (char*)&ib_globals.buffer;; 
...
int main(int argc, char**argv)
{
	...
	if(strcmp(argv[4],"0") ==0) detection =1;
	if(strcmp(argv[4],"1") ==0) protection =1;
	...
	if(protection ==1)
	{
		...
		file_write =  (char *) malloc(sizeof(char)*100);	
		file_read =  (char *) malloc(sizeof(char)*100);	
		strcpy(file_write, argv[2]);
		strcpy(file_read, argv[3]);
		strcpy(hostname, argv[1]);
		toto =pthread_create(&pthread_get, NULL, thread_get, NULL);
		pthread_join(pthread_get, NULL);
	}
